...
const SECTION_NAME = "EngineTransmission";
...
const EngineTransmissionInspection = ({ navigation, route }) => {
  ...
  const components = useMemo(
    () => [
      { key: "battery", label: "Battery", type: "image" },
      { key: "engineOilLevelDipstick", label: "Engine Oil Level Dipstick", type: "image" },
      { key: "engineOil", label: "Engine Oil", type: "image" },
      { key: "coolant", label: "Coolant", type: "image" },
      { key: "engineMounting", label: "Engine Mounting", type: "image" },
      { key: "engineSound", label: "Engine Sound", type: "video" },
      { key: "exhaustSmoke", label: "Exhaust Smoke", type: "video" },
      { key: "turboCharger", label: "Turbo Charger", type: "choice" },
      { key: "fuelInjector", label: "Fuel Injector", type: "choice" },
      { key: "clutch", label: "Clutch", type: "choice" },
      { key: "gearShifting", label: "Gear Shifting", type: "choice" },
    ],
    []
  );

  // ───────── Hydrate selected options from Firestore + queue ─────────
  useEffect(() => {
  ...
  const saveMediaPermanently = async (uri, id, type) => {
    const ext = type === "video" ? "mp4" : "jpg";
    const newPath = `${FileSystem.documentDirectory}${inspectionId}_${SECTION_NAME}_${id}.${ext}`;
    await FileSystem.copyAsync({ from: uri, to: newPath });
    return newPath;
  };

  const handleYesWithCamera = async (componentKey, type) => {
    try {
      ...
      if (!result.canceled && result.assets?.length > 0) {
        ...
        const localPath = await saveMediaPermanently(tempUri, id, type);
        ...
        setSelectedOptions((prev) => ({ ...prev, [componentKey]: "Yes" }));
      }
    } catch (err) {
      ...
      if (option === "No") {
        navigation.navigate("ComponentIssueScreen", {
          ...
          onIssueSaved: (issueData) => {
            ...
            setSelectedOptions((prev) => ({ ...prev, [componentKey]: "No" }));
          },
        });
      } else {
        dispatch(
        ...
        setSelectedOptions((prev) => ({ ...prev, [componentKey]: option }));
      }
    } else {
      if (option === "No") {
        navigation.navigate("ComponentIssueScreen", {
        ...
        handleYesWithCamera(componentKey, type);
      }
    }
  };
  ...
  const YesNoButton = ({ label, componentKey, type }) => {
    ...
    const hasMedia = !isChoice && (section?.image || status);
      ...
      <View style={styles.componentContainer}>
        ...
        {hasMedia && (
          <View style={styles.uploadedContainer}>
            ...
            <Text style={{ color: "green", marginLeft: 5 }}>File Added</Text>
          </View>
        )}
